...
larger jobs. \jhanote{slightly misleading to claim there are three queues then
to say immediately there is no backfill queue. Haven't we alread discussed
this in Section 3?} The batch queue is the default queue for submitted jobs,
...
\jhanote{Please make the formulation of the statement more precise} Recall
that the goal of CSC108 is to consume idle resources that will otherwise go to
waste. 
\jhanote{the following is a very strong statement. Discuss} As shown in
...
effort not to disturb the rest of Titan's ecosystem. \jhanote{Please be
specific about the ecosystem, viz., the QOS for other users as measured by
their average wait time.} The results of
...
Titan and search for simple linear relationships \jhanote{of what??}
especially direct and inverse relationships, by using ordinary least-squares
regression. \jhanote{unclear opening. I think it needs rewrite.}
